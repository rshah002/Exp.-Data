...
if __name__ == '__main__':
    ...
    plt.title("Third Harm. K=%s, I=%s mA" %(a0,I))
    ...
    plt.plot(K_2_12_harm_2x, K_2_12_harm_2y, color = color2, label=r'exp. data')
    #plt.set_yscale('log')
    plt.xlabel('Photon Wavelength  (Angstrom)')
    plt.ylabel("dN/dΩ [photons/(s mrad^2 0.1%BW)]")
    plt.xlim(30.4, 31.4)
    plt.legend()
    plt.savefig('plt_XSENSE2_intensity_K_2.12_third_real.eps', format='eps', dpi=2000)
    plt.show()
